...
import GenLogic from "../../common/GenLogic";
import Status from "../../common/Status";
import Util from "../../common/Util";
import SequenceEvent from "../../common/SequenceEvent";

const basic = (WrappedComponent, logic) => class extends WrappedComponent {
    ...
    genChildren(element) {
        if (Util.isArray(element) && element.length) { //如果是数组就循环解析元素
            ...
            if (children && Util.isArray(children) && children.length) {//检测是否有子元素，有就继续循环解析元素
                props = props || {};
        ...
        data['list'].forEach((v,k)=>{ //一个事件对应一个对象，这个事件引起的值变化，激励响应都保存其中
            ...
            seq.events.push((info,oldValue,newValue)=>{ //第二个方法，根据新值的改变调用方法产生激励
                ...
                Object.keys(info['movt']).forEach((mn)=>{ //循环调用引起的激励产生的事件
                    if(info['movt'][mn]){
                        mev = this.bone.props['on'+Util.upFirstWord(mn)];
                ...
                this.setState({
                    ...
                    let onChanged = this.bone.props['onChanged']; //检测并调用onchange方法
